export const ernaehrungFragenA1 = [

    {
        hintergrund: "./Bilder/Ampel/Ernährung/Level1/Hintergrund.png",
        items: [
            "./Bilder/Ampel/Ernährung/Level1/apfel_biobayern.png",
            "./Bilder/Ampel/Ernährung/Level1/apfel_bioitalien.png",
            "./Bilder/Ampel/Ernährung/Level1/apfel_neuseeland.png"
        ],
        koords: [
            { bottom: "35%", left: "5%" },
            { bottom: "35%", left: "35%" },
            { bottom: "35%", left: "65%" },
        ],
        type: "ampel",
        question: "Du willst dir in der Pause am Kiosk Obst kaufen. Du kaufst …",
        answers: [
            { text: "… einen Apfel aus der Region.", correct: "correct" },
            { text: "… einen Bio-Apfel aus Italien.", correct: "ok" },
            { text: "… einen Apfel aus Neuseeland. ", correct: "wrong" },
        ],
        tipp: "Region",
        expl: "Ein Apfel aus der Region verursacht weniger Treibhausgase, da er nicht so weit transportiert werden muss. Ein Bio-Apfel wird zwar mit weniger Pestiziden behandelt, jedoch ist ein Transport im LKW deutlich schlechter für's Klima."
    },
]

export const ernaehrungFragenA2 = [
    {
        hintergrund: "./Bilder/Ampel/Ernährung/Level2/Hintergrund.png",
        items: [
            "./Bilder/Ampel/Ernährung/Level2/flasche3.png",
            "./Bilder/Ampel/Ernährung/Level2/flasche2.png",
            "./Bilder/Ampel/Ernährung/Level2/flasche1.png"
        ],
        koords: [
            { bottom: "35%", left: "5%" },
            { bottom: "35%", left: "35%" },
            { bottom: "35%", left: "65%" },
        ],
        type: "ampel",
        question: "Du möchtest dir etwas zu Trinken kaufen. Du kaufst …",
        answers: [
            { text: "… eine Mehrweg-Glasflasche.", correct: "ok" },
            { text: "… eine Alu-Getränkedose.", correct: "wrong" },
            { text: "… eine Mehrweg-Plastikflasche.", correct: "correct" },
        ],
        expl: "Eine Glasflasche kann bis zu 50 mal wiederbefüllt werden, eine Plastikflasche nur etwa 25 mal. Wenn die Plastikflasche einen längeren Transportweg hat, dann schneidet sie durch ihr geringeres Gewicht besser ab als die Glasflasche. Wenn du aber zum Beispiel ein Getränk eines regionalen Herstellers kaufst, ist die Glasflasche in diesem Fall besser."
    },
]

export const ernaehrungFragenA3 = [
    {
        hintergrund: "./Bilder/Ampel/Ernährung/Level3/Hintergrund.png",
        items: [
            "./Bilder/Ampel/Ernährung/Level3/pommes.png",
            "./Bilder/Ampel/Ernährung/Level3/käsesemmel.png",
            "./Bilder/Ampel/Ernährung/Level3/CHeeseburger.png"
        ],
        koords: [
            { bottom: "35%", left: "5%" },
            { bottom: "35%", left: "35%" },
            { bottom: "35%", left: "65%" },
        ],
        type: "ampel",
        question: "In der großen Pause hast du richtig Hunger. Du kaufst …",
        answers: [
            { text: "… eine Portion Pommes Frites (tiefgekühlt). ", correct: "correct" },
            { text: "… ein Käsebrötchen.", correct: "ok" },
            { text: "… einen Hamburger (tiefgekühlt). ", correct: "wrong" },
        ],
        expl: "Generell sind Tiefkühllebensmittel schlechter für's Klima, da die Kühlung viel Energie kostet. Weil die Herstellung von Rindfleisch und Käse viel Energie benötigt, die Kühe Futter brauchen und Methan bei der Verdauung ausstoßen, schneiden die tiefgekühlten Pommes trotzdem am besten ab."
    },
]

export const ernaehrungFragenA4 = [
    {
        hintergrund: "./Bilder/Ampel/Ernährung/Level4/Hintergrund.png",
        items: [
            "./Bilder/Ampel/Ernährung/Level4/Milch.png",
            "./Bilder/Ampel/Ernährung/Level4/Hafermilch.png",
            "./Bilder/Ampel/Ernährung/Level4/Sojamilch.png"
        ],
        koords: [
            { bottom: "35%", left: "5%" },
            { bottom: "35%", left: "35%" },
            { bottom: "35%", left: "65%" },
        ],
        type: "ampel",
        question: "Beim Bäcker willst du dir in der Pause eine heiße Schokolade kaufen. Zum Glück hat dein Bäcker mehrere Alternativen: ",
        answers: [
            { text: "Eine Tasse heiße Schokolade mit Kuhmilch.", correct: "wrong" },
            { text: "Eine Tasse heiße Schokolade mit Hafermilch.", correct: "correct" },
            { text: "Eine Tasse heiße Schokolade mit Sojamilch.", correct: "ok" },
        ],
        expl: "Tierische Lebensmittel, also auch Kuhmilch, verbrauchen in der Produktion viel Energie, es muss Futter für die Kuh angebaut werden, und sie stößt bei der Verdauung das klimaschädliche Gas Methan aus. Ein Glas pflanzliche Hafer- oder Sojamilch verursacht im Vergleich nur etwa ein Drittel der CO2-Emissionen."
    },

]

export const ernaehrungFragenA5 = [
    {
        hintergrund: "./Bilder/Ampel/Ernährung/Level5/Hintergrund.png",
        items: [
            "./Bilder/Ampel/Ernährung/Level5/vegan.png",
            "./Bilder/Ampel/Ernährung/Level5/traditionell.png",
            "./Bilder/Ampel/Ernährung/Level5/veggie.png"
        ],
        koords: [
            { bottom: "35%", left: "5%" },
            { bottom: "35%", left: "35%" },
            { bottom: "35%", left: "65%" },
        ],
        type: "ampel",
        question: "Am Wochenende gehst du frühstücken. Du entscheidest dich für …",
        answers: [
            { text: "… Das vegane Frühstück: \n 1 Glas frisch gepresster Orangensaft, 2 Scheiben Vollkornbrot mit Margarine und Honig, einen Obstsalat mit Apfel, Birne und Banane.", correct: "correct" },
            { text: "… Das traditionelle Frühstück: \n 1 Glas Milch, 2 gekochte Eier, 1 Scheibe Brot mit Butter und Schinken, eine Scheibe Brot mit Butter und Käse.", correct: "ok" },
            { text: "Das vegetarische Frühstück: \n 1 Glas frisch gepresster Orangensaft, 1 Schale Joghurt mit Haferflocken und einer halben Mango, 1 Scheibe Brot mit Butter und Honig und 1 gekochtes Ei.", correct: "wrong" },
        ],
        expl: "Das traditionelle Frühstück verursacht ungefähr 1 Kilogramm CO2. Das vegetarische Frühstück allerdings noch mehr, denn Joghurt, Eier und Butter sind alles tierische Produkte, die besonders energieintensiv hergestellt werden müssen. Das vegane Frühstück verursacht weniger als die Hälfte an klimaschädlichen Treibhausgasen."
    },

]


